...
$$ \frac{d}{dK} [f(K) ⊙ C] = f'(K) \cdot \frac{1}{(1 + f(K)C)^2} $$
...
def solve_riccati_via_kmr(P, Q, R, K_vals, y0):
    ...
    from kmr_operations import kmr_dircly # Импорт функции прямого оператора
    ...
    for i in range(1, len(K_vals)):
        ...
        if np.abs(R[i] * P[i] - R[i-1] * P[i-1]) < 1e-10:
            ...
            y[i] = kmr_dircly(y[i-1], R[i] * K)
        else:
            # Approximate using KMR step
            y[i] = kmr_dircly(y[i-1], R[i] * K) + Q[i] * K
    return y
 ```
